  ...
  def _value_fromdict(self, key, value):
    if not isinstance(value, YearlyBase):
      if not isinstance(value, dict):
        raise ValueError('Value must a YearlyBase based class or a dict')
...
class YearlyAttrProxy(object):
  ...
  def update(self, value):
    if isinstance(value, YearlyAttrProxy):
      ...
      raise TypeError('Except a YearlyAttrProxy or dict instance, got: {!r}'
...
class YearlyAttribute(object):
  ...
  def __repr__(self):
    return '<{}{!r} at 0x{:x}>'.format(self.__class__.__name__,
                                         self._attrs,
                                         id(self))
